...
const studentsSql = {
  ...
  getCollegeCount:'SELECT collegeId, count( * ) AS cntNum FROM s_students GROUP BY collegeId ORDER BY cntNum desc',
  getSexCount:'SELECT collegeId, count( * ) AS cntNum FROM s_students where sex=? GROUP BY collegeId ORDER BY cntNum desc',
  queryAgeCount:'SELECT age, count( * ) AS cntNum FROM s_students where sex=? GROUP BY age ORDER BY age ASC'
}
const counselorSql = {
